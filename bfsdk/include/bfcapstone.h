...
namespace capstone
{
    ...
    uint8_t read8(const save_state_t *state, uint64_t byte_offset)
    {
        expects(byte_offset <= 0x80U);
        return reinterpret_cast<const uint8_t *>(state)[byte_offset];
    }

    uint16_t read16(const save_state_t *state, uint64_t byte_offset)
    {
        expects(byte_offset <= 0x80U);
        return reinterpret_cast<const uint16_t *>(state)[byte_offset >> 1U];
    }


    uint32_t read32(const save_state_t *state, uint64_t byte_offset)
    {
        expects(byte_offset <= 0x80U);
        return reinterpret_cast<const uint32_t *>(state)[byte_offset >> 2U];
    }


    uint64_t read64(const save_state_t *state, uint64_t byte_offset)
    {
        expects(byte_offset <= 0x80U);
        return reinterpret_cast<const uint64_t *>(state)[byte_offset >> 3U];
    }

    void write8(save_state_t *state, uint64_t byte_offset, uint8_t val)
    {
        expects(byte_offset <= 0x80U);
        reinterpret_cast<uint8_t *>(state)[byte_offset] = val;
    }

    void write16(save_state_t *state, uint64_t byte_offset, uint16_t val)
    {
        expects(byte_offset <= 0x80U);
        reinterpret_cast<uint16_t *>(state)[byte_offset >> 1U] = val;
    }

    void write32(save_state_t *state, uint64_t byte_offset, uint32_t val)
    {
        expects(byte_offset <= 0x80U);
        reinterpret_cast<uint32_t *>(state)[byte_offset >> 2U] = val;
    }

    void write64(save_state_t *state, uint64_t byte_offset, uint64_t val)
    {
        expects(byte_offset <= 0x80U);
        reinterpret_cast<uint64_t *>(state)[byte_offset >> 3U] = val;
    }

//    void cstone_open(csh *cstone)
//    {
//        const cs_err err = cs_open(CS_ARCH_X86, CS_MODE_64, cstone);
//        if (err != CS_ERR_OK) {
//            bferror_nhex(
//                0,
//                "capstone::open unable to open handle, errno = ",
//                static_cast<uint64_t>(err));
//
//            throw std::runtime_error(
//                "capstone::open unable to open handle, errno = " +
//                std::to_string(static_cast<uint64_t>(err)));
//        }
//
//        cs_option(*cstone, CS_OPT_DETAIL, CS_OPT_ON);
//    }
//
//    uint64_t cstone_disasm_exit_insn(
//        csh cstone,
//        const uint8_t *code,
//        uint64_t addr,
//        cs_insn **insn)
//    {
//        const auto nr_needed = 1U;
//        const auto nr_bytes = vmcs_n::vm_exit_instruction_length::get();
//        expects(nr_bytes <= 15U);
//
//        const auto nr_disasm = cs_disasm(
//            cstone, code, nr_bytes, addr, nr_needed, insn);
//
//        if (nr_disasm != nr_needed) {
//            bferror_info(0, "Expect: nr_disasm == nr_needed");
//            bferror_subnhex(0, "nr_disasm", nr_disasm);
//            bferror_subnhex(0, "nr_needed", nr_needed);
//            bferror_subnhex(0, "nr_bytes", nr_bytes);
//
//            for (auto i = 0U; i < nr_bytes; ++i) {
//                printf("%02x", code[i]);
//            }
//            printf("\n");
//
//            throw std::runtime_error("Expect: nr_disasm == nr_needed");
//        }
//    }
//
//    void cstone_check_xapic_write_ops(cs_insn *insn)
//    {
